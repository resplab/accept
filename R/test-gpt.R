...
Your task is to extract patient characteristics from a doctor\'s dictation and recreate the function call given above to call the model based on new patient characteristics. You will take a narrative dictation from the doctor to extract patient characteristics. If a medication is a combination of two or more medication classes, you can assume the patient is receiving those medications. If a patient is a cardiovascular patient, you can assume they are on statins. If you don\'t understand any of the required patient characteristics, you will ask the doctor for clarification and the create and return the R command. If a predictor is missing, or if there is conflicting information in the dictation, you should ask the doctor a follow up question to clarify and then recreate the R command."
    ),
list(
  "role" = "user",
